...
googletag.pubads().refresh();};function initAdserver(){if(prebidJs.initAdserverSet)return;prebidJs.initAdserverSet=true;googletag.cmd.push(function(){prebidJs.que.push(function(){prebidJs.setTargetingForGPTAsync();googletag.pubads().refresh();});});}
...
var prebidTargetting=function(response){var config=prebidJs.getConfig();var user=Object.assign(config.user||{},response.user);user.dnt=win._ccScriptSettings.user.dnt;prebidJs.setConfig({user:user});prebidJs.que.push(function(){prebidJs.requestBids({timeout:PREBID_TIMEOUT,bidsBackHandler:initAdserver});});};win.ccRealTime=win.ccRealTime||{};win.ccRealTime.callback=function(response){clearTimeout(requestTimeout);if(!requestTimedOut&&win._ccScriptSettings.realtime.setGptTargeting&&googletag.pubads){gptTargeting(response);}
...
if(win._ccScriptSettings.realtime.loadAudiences){win._ccScriptSettings.user.audienceMemberships=response.audiences.map(function(i){return i.id;});win.cca.api.dispatchEvent('audiencesReady',response.audiences.map(function(i){return i.id;}));}
...
if(win._ccScriptSettings.realtime.loadBrands){win._ccScriptSettings.user.brands=response.brands;win.cca.api.dispatchEvent('brandsReady',response.brands);}};function addDays(date,days){var result=new Date(date);result.setDate(result.getDate()+days);return result;}
var startScript=function(userId,isNew){if(isNew){return;}
requestTimeout=setTimeout(50,function(){requestTimedOut=true;});var qsOptions="?ccuid="+userId+
